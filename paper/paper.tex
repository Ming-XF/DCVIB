...
% 3.3 真实压缩-精度曲线（横轴 = 实际 E[KL]，图 fig:compression-*）/
% 3.4 对抗鲁棒性 \input{result2} / 3.5 机制验证 \input{result3} + 图）、
% 语句小节、参考文献、附录（\input{appendix.tex}：诊断/证明/构造细节/补充结果、
% 超参数敏感性表 result1、KL 分解表 result4）。相关工作为占位。
...
answers both on twelve dataset--backbone settings; the compression--accuracy curves
at the realized $\E[\KL]$ are reported in Section~\ref{sec:sweep},
...
\subsection{Compression--accuracy at the realized KL}
\label{sec:sweep}

The main table selects the best $(\beta, a)$ per method; the curves below report
the \emph{realized} compression--accuracy trade-off on one classification and one
regression setting (ImageNet-100 and Cal. Housing, both with the MLP backbone).
The horizontal axis is the test-set $\E[\KL]$ --- the certificate itself --- so
$\beta$ only parameterizes the curves. This matters because the two methods reach
different compressions at the same $\beta$: on Housing at $\beta = 0.1$, CEB
already sits at $\E[\KL] \approx 0.09$ while OPB ($a = 12$) is at $\approx 0.27$,
so a $\beta$-matched comparison would confound compression level with geometry,
whereas the KL-matched comparison isolates the geometry. The $\beta$-grid tables
and the full per-configuration KL decomposition are in
Appendix~\ref{app:hparam-tables}.

\begin{figure*}[t]
\centering
\includegraphics[width=0.48\linewidth]{figures/fig_compression_imagenet100.png}\hfill
\includegraphics[width=0.48\linewidth]{figures/fig_compression_housing.png}
\caption{Compression--accuracy against the realized test-set $\E[\KL]$ (log
scale), $\beta \in \{5{\cdot}10^{-5}, \dots, 25\}$ parameterizing each curve.
(a) ImageNet-100 (MLP): test accuracy; OPB with $a = 12$ holds the plateau while
CEB and OPB ($a = 1$) collapse. (b) Cal. Housing (MLP): test $R^2$. Collapsed
corner points ($\E[\KL] < 10^{-3}$), dead-prediction endpoints (accuracy below
$5\%$ / $R^2$ below $0.1$), and variance-explosion points ($\E[\KL] > 10^{4}$)
are omitted.}
\label{fig:compression}
\end{figure*}

Figure~\ref{fig:compression}
exhibits the two roles of the anchor scale on the realized compression axis.
\emph{(i) Without a scale, the reference collapses.} On ImageNet-100, CEB's
accuracy erodes to $71.5\%$ at $\E[\KL] \approx 3.6$ ($\beta = 0.1$) and its
reference escapes entirely at $\beta \ge 0.5$ --- $\E[\KL] \to 0$ with accuracy at
chance --- so its curve ends there; OPB at $a = 1$ collapses at the same end of
the axis. On Housing, CEB drops to $R^2 = 0.49$ at $\beta = 0.1$ and below $0.1$
by $\beta \ge 0.5$, while every OPB curve keeps $R^2 \ge 0.76$ out to
...
or ties.} On ImageNet-100, OPB ($a = 12$) attains $84.2\%$ at
$\E[\KL] \approx 21$ --- above CEB's best point anywhere on its curve
($82.8\%$ at $\E[\KL] \approx 26$) --- and still $79.4\%$ at $\beta = 25$
($\E[\KL] \approx 2.2$); on Housing the four curves peak within half a point
(CEB $0.789$, OPB $0.786$--$0.789$), but only OPB retains its peak into the
strong-compression regime: the plateau is the flat OPB segment where $R^2$ stays
above $0.76$ while $\E[\KL]$ shrinks by two orders of magnitude.

A visible cost remains in the weak-compression regime: on ImageNet-100 at
$\E[\KL] \gtrsim 300$, OPB trails CEB by one to three points ($a = 6$: $81.0$ vs.\
$82.6$; $a = 12$: $79.8$ vs.\ $82.6$) --- the price of the fixed-scale energy rule
when compression is weak. The construction pays for itself in the
strong-compression regime, where CEB is at chance.
...
a plateau dial. The KL decomposition (Table~\ref{tab:compression_decomp} in
Appendix~\ref{app:hparam-tables}) shows the two channels at work: OPB's $\E[\KL]$ is
