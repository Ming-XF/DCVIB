...
variation in the input $X$ that does not support prediction. The balance matters
because compression acts as a principled regularizer: discarding label-irrelevant
variation is expected to improve generalization and robustness to nuisances, and the
compressed representation exposes the minimal structure sufficient for the task,
which has motivated a sustained line of deep IB methods
\citep{fischer2020robustness,lee2021compressive,zhai2022adversarial,wang2021hsic}.
Variational IB (VIB)
...
compares two distinct label conditionals. Consequently, class means, directions, and
pairwise margins are determined by optimization dynamics and prediction-head freedom,
rather than by the compression criterion. The deterministic-label corner phenomenon
identified in prior work \citep{kolchinsky2019caveats} is one manifestation of this
freedom; a complementary
...
different object. Geometric structure has been imposed before, on features,
classifier weights, or prototypes
\citep{deng2014equidistant,papyan2020prevalence,xu2022orthogonal,tong2023fixed_frame},
but always outside the compression term: those methods couple geometry to a
classification objective, while the bottleneck, when present, remains blind to it.
GPB instead declares the geometry of the conditional prior itself, so the same
declared structure enters the KL that compresses the representation and the readout
that predicts from it. The detailed comparison with each line of work, together
with the boundary table, is given in Appendix~\ref{sec:related}.
