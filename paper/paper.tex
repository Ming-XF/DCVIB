...
the geometry between different labels unspecified. We introduce a geometric prior
bottleneck (GPB) that declares this geometry statelessly: OPB uses an orthogonal
...
GPB preserves the CEB certificate and yields a comparator-dependent alignment
inequality whose floor decomposes label ambiguity, encoder approximation, and
covariance mismatch; a separate mismatch-conditioned result transfers prior
separation to posterior centers. Controlled label-noise, direct mismatch,
...
The information bottleneck (IB) principle~\citep{tishby2000information} formulates
representation learning as a balance between predictive sufficiency and compression:
the representation $Z$ should retain information about a target $Y$ while discarding
variation in the input $X$ that does not support prediction. Variational IB (VIB)
~\citep{alemi2017deep} makes this trade-off trainable with a stochastic encoder and
a marginal reference distribution. The conditional entropy bottleneck (CEB)
~\citep{fischer2020conditional} replaces that marginal reference with a
label-conditional distribution and thereby targets the residual information
$I(X;Z\mid Y)$. Its variational certificate,
\begin{equation*}
\E\KL(q(z|x)\|b(z|y))\ge I(X;Z\mid Y),
\end{equation*}
is attractive because the reference can adapt to the label structure rather than
forcing every class toward a common origin.

The same flexibility creates a geometric gap. CEB matches each posterior to the
reference associated with its own label, but the certificate contains no term that
compares two distinct label conditionals. Neither the marginal-reference VIB nor its
variants constrain that geometry either: noise-injection bottlenecks replace the
bound \citep{kolchinsky2017nonlinear}, bound comparisons clarify reference choices
\citep{geiger2020variational}, multivariate extensions add decoders
\citep{abdelaleem2025deep}, and robustness deployments apply the same view at scale
\citep{fischer2020robustness,lee2021compressive}. Consequently, class means,
directions, and pairwise margins are determined by optimization dynamics and
prediction-head freedom, rather than by the compression criterion. The
deterministic-label corner phenomenon identified in prior work
\citep{kolchinsky2019caveats} is one manifestation of this freedom; a complementary
issue persists even when labels are noisy or the information-plane knob moves: the
certificate can be tight while the inter-label arrangement remains arbitrary. On the
$T_\alpha=B_\alpha\!\cdot\!Y$ family, for example, the conditional residual can stay
zero while retained label information changes across the family
(Proposition~\ref{prop:geom-blind}; the full construction is in Appendix~\ref{app:diagnosis}).
This observation motivates a prior whose geometry is part of the model definition.

We introduce the geometric prior bottleneck (GPB), a framework for
information-bottleneck compression in which the conditional reference is a declared
geometric prior rather than an unconstrained one: a stateless conditional Gaussian
with bounded covariance whose means obey a two-sided declared geometry, fixed before
training. The framework, not any single instance, is the object of the theory in
this paper: the gap identity holds for every stateless conditional prior, and the
alignment and separation results hold for any qualified geometric prior. We
instantiate the framework with two concrete constructions. The orthogonal prior
bottleneck (OPB) declares the classification geometry as an equal-radius orthogonal
frame and uses the corresponding Gaussian anchor energies as the classifier. The
equidistant prior bottleneck (EPB) declares the regression geometry as an isometric
axis and uses the same axis for prediction. Thus each prior mean has two
simultaneous roles: it is a target in the KL and an anchor in the readout. Both
constructions are computed from the current all-label prior table, with no EMA,
prototype memory, or batch-dependent geometry, and other declared geometries
instantiate the same framework.

GPB reuses geometric primitives from several lines of work but attaches them to a
different object. Prototype and neural-collapse methods constrain features,
prototypes, or classifier weights through a classification loss or a fixed readout
\citep{deng2014equidistant,papyan2020prevalence,zhu2021geometric_nc,koromilas2026neural,wang2023ib_nc};
orthogonal and ETF classifiers constrain the readout
\citep{xu2022orthogonal,ji2023etf}; fixed frames constrain feature coordinates
\citep{tong2023fixed_frame}. OPB introduces none of these objectives: its frame is
the mean map of a label-conditional Gaussian prior, and its anchor-energy readout is
the Bayes rule induced by the shared covariance, so the geometric class code and the
conditional KL are coupled during training. Robustness-oriented bottlenecks modify
the information criterion or add adversarial objectives
\citep{wang2021hsic,zhai2022adversarial,zhang2022nl_ib,hua2022causal}; GPB keeps the
ordinary CEB KL and instead transfers a declared prior geometry. Distance-preserving
methods constrain deterministic feature maps \citep{pai2022isometric,luo2023doll},
and regression compression objectives add bounds rather than geometry
\citep{yu2024csd_ib,belucci2026adacap}; EPB places the label metric in the
conditional prior, whose mean map is also the tied regression readout. Recent
geometry-aware bottlenecks use the same phrase for other objects
\citep{yan2024geometric_ib,katende2025vgib,doan2026geometric_ib,adilova2026geometric_compression}.
The boundary is thus at the framework level rather than the instance level: existing
geometric methods are fixed constructions, while GPB is the class of IB objectives
whose conditional prior carries a declared geometry, and OPB and EPB are two
instances of that class evaluated in this paper. The object-level distinction is
summarized in the boundary table of Appendix~\ref{sec:related}.

The construction leads to two complementary theoretical statements. First, the CEB
gap identity continues to hold for every stateless conditional prior. Second, for a
near-optimal solution and any feasible comparator $w_0$,
\begin{equation*}
\mathcal D(\widehat w)\le \mathcal K(w_0)+
\frac{\mathcal R(w_0)+\varepsilon}{\beta},
\end{equation*}
so alignment is governed by a comparator's KL floor and prediction risk. The floor
decomposes into irreducible label ambiguity, encoder mean approximation, and covariance
mismatch (Appendix~\ref{app:deferred-theory}). Independently, for any qualified
geometric prior the mismatch-conditioned separation proposition converts the realized
distance to the declared prior anchors into a lower bound on posterior-center
separation. Together these results connect the
constructed geometry to both optimization and the representation actually produced by
the encoder.

We evaluate GPB through complementary evidence. The main table compares GPB with the
complete IB and non-IB baseline set on twelve dataset--backbone settings. The controlled
label-noise study follows the predicted zero-to-nonzero ambiguity floor; the direct
mismatch study measures how much of the declared geometry reaches the posterior; and
the compression, information-plane, robustness, and mechanism analyses examine the
behavioral consequences of that transfer. Detailed uncertainty, regression curves,
transfer attacks, pair-level diagnostics, and construction checks are provided in the
corresponding appendices~\ref{app:construction}--\ref{app:main-ci}.

In summary, our contributions are: (i) identifying and formalizing the missing
cross-label geometric constraint in CEB; (ii) introducing GPB with OPB and EPB, whose
prior means serve simultaneously as KL targets and prediction anchors; (iii) deriving
comparator-dependent alignment and mismatch-conditioned separation results with an
explicit ambiguity--approximation decomposition; and (iv) providing controlled and
benchmark evidence that separates geometric mechanism, compression behavior, and
end-to-end utility.
...
holds for every stateless conditional prior. The posterior-separation inequality holds
for the mismatch realized by any fitted model. The optimization-dependent statement is an
...
\begin{proposition}[Mismatch-conditioned posterior separation for any qualified geometric prior]
...
\paragraph{When the separation inequality is informative.}
The proposition is a conditional stability statement, not a claim that every
qualified prior produces a non-vacuous posterior bound. For a discrete pair, define
the normalized mismatch radius
\begin{equation}
\Gamma_{ij}:=
\frac{\sqrt{2\tau^2_{\max}D_i}+\sqrt{2\tau^2_{\max}D_j}}
{\|m_\theta(i)-m_\theta(j)\|}.
\label{eq:separation-tightness-ratio}
\end{equation}
The raw lower bound is positive exactly when $\Gamma_{ij}<1$, and in that case it
retains a fraction $1-\Gamma_{ij}$ of the declared pairwise distance. For OPB this
condition becomes
$\tau_{\max}(\sqrt{D_i}+\sqrt{D_j})<a$; if $D_i=D_j=D$, it reduces to
$D<a^2/(4\tau^2_{\max})$. For regression, the corresponding condition is
\begin{equation*}
\sqrt{2\tau^2_{\max}D(y)}+\sqrt{2\tau^2_{\max}D(y')}
<\rho_{\min}d_Y(y,y'),
\end{equation*}
so nearby label pairs are inherently more likely to have a vacuous lower bound.
Increasing the anchor scale does not by itself enforce these conditions because it
also changes the fitted mismatch and the optimization problem. The proposition
therefore neither selects $a$ or $\rho$ nor guarantees that a validation-selected
scale has a positive bound.
...
\paragraph{When the comparator bound is informative.}
Theorem~\ref{prop:pgib-alignment} is an oracle inequality relative to a specified
feasible comparator; it is not a prediction determined by the dataset and $\beta$
alone. For a desired mismatch level $d_\star$, its right-hand side can certify
$\mathcal D(\widehat w)\le d_\star$ only if there is a comparator with
$\mathcal K(w_0)<d_\star$. A sufficient condition is
\begin{equation}
\beta\ge
\frac{\mathcal R(w_0)+\varepsilon}
{d_\star-\mathcal K(w_0)}.
\label{eq:comparator-informative-condition}
\end{equation}
Thus $\mathcal K(w_0)$ is the floor of this comparator-specific certificate: increasing
$\beta$ cannot certify a target below it, and a pure rate to zero requires a
zero-KL comparator.

For a fixed comparator $w_0$ chosen independently of a held-out sample
$S=\{(x_i,y_i)\}_{i=1}^n$, its two observable terms admit the plug-in estimates
\begin{equation}
\widehat{\mathcal K}_S(w_0)=\frac1n\sum_{i=1}^n
\KL\!\left(q_{\phi_0}(z|x_i)\,\|\,p_{\theta_0}(z|y_i)\right),
\qquad
\widehat{\mathcal R}_S(w_0)=\frac1n\sum_{i=1}^n
\E_{q_{\phi_0}(z|x_i)}[\ell(c_{\psi_0}(Z),y_i)].
\label{eq:heldout-comparator-estimates}
\end{equation}
The Gaussian KL is analytic, and under the usual integrability assumptions these
sample averages consistently estimate the chosen comparator's population terms.
They do not identify the best comparator, certify the global optimization error
$\varepsilon$, or by themselves produce a finite-sample population guarantee. In
particular, choosing the reported solution itself as $w_0$ after seeing the results
makes the comparison nearly tautological. Moreover, on a benchmark with one observed
label per input, $P(Y\mid X)$ and hence the separate ambiguity term in
Proposition~\ref{prop:pgib-ambiguity-decomposition} are not identifiable without
additional annotation or modeling assumptions. What can be estimated without that
decomposition is the total KL of a prespecified feasible comparator; it should be
described as a comparator-specific floor, not an intrinsic floor of the dataset.
...
$S_t:=\{y:D(y)\le t\}$ has $P_Y$-measure at least $1-B_0/t$, and the pointwise inequality of
Theorem~\ref{thm:pgib-separation}(b) yields, for $P_Y$-a.e. $y,y'\in S_t$,
\begin{equation}
...
The experiments are organized into three evidence tracks. The first is a predictive
comparison: it reports end-to-end performance for CEB, GPB, and the other baselines,
including a tied-readout variant, under the same selection protocol. The main sweep
reports these comparisons on twelve dataset--backbone
settings, with compression--accuracy curves in Section~\ref{sec:sweep} and adversarial
robustness in Section~\ref{sec:robustness}. The second is structural analysis:
Section~\ref{sec:mechanism} reports geometric diagnostics and directly analyzes the
...
differences in the first track are not assigned to the theoretical bounds in the other
two.
...
\paragraph{Separation of performance and structural diagnostics.} The main benchmark
...
approximation terms. The benchmark protocol does not designate an independent feasible
comparator, so it does not report twelve post-hoc ``floor estimates'': using the fitted
solution itself would be nearly tautological. Equation~\eqref{eq:heldout-comparator-estimates}
instead gives a held-out estimation procedure when a comparator is prespecified. We do
not infer an alignment rate from test accuracy. The structural analysis separately
reports posterior geometry and tied-readout performance. Finally, the controlled
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.24\linewidth]{figures/fig_noisy_a_dhat_beta.png}\hfill
\includegraphics[width=0.24\linewidth]{figures/fig_noisy_b_excess_invbeta.png}\hfill
\includegraphics[width=0.24\linewidth]{figures/fig_noisy_c_beta_excess.png}\hfill
\includegraphics[width=0.24\linewidth]{figures/fig_noisy_d_acc_beta.png}
...
Appendix~\ref{app:main-std}. Four
observations.

\emph{Predictive comparison under the shared protocol.} GPB is best or
...
advantage. GPB has a higher mean than CEB in all $12$ settings and is best among all
methods on seven rows.
The largest positive differences occur on the high-cardinality or lower-data settings:
...
rows where the plain backbone remains best (STS-B, AG News) the difference is
marginal ($\le 0.03$). On
...
accuracy of a freely trained head in almost all tested settings. This shows that the
removed degrees of freedom are not necessary to reproduce those particular results;
it is not a claim about all data distributions. The three settings where GPB-L edges out GPB by mean (IMDb, AG News,
...
\emph{Tied and free readouts by regime.} The two settings with the largest GPB
margins are ImageNet-100, with $K=100$, and AgeDB, a small, high-variance regression
task. This pattern is consistent with the fixed-scale condition under which the
aligned surrogate of Proposition~\ref{prop:opb-sweep} has a unique interior optimum
per $\beta$, but the cross-dataset comparison does not identify regularization by the
tied rule as the cause of either margin.
...
\subsection{Factor-wise attribution ablations}
\label{sec:ablation}

The GPB construction bundles three design factors --- the orthogonal/isometric
geometry, the fixed anchor scale, and the tied geometric readout --- with the
IB objective itself, and the main table alone cannot compare their marginal
associations with performance. This section switches each factor separately. All
variants are
...
of the structural analysis, and the reference models (Base, CEB, GPB, GPB-L)
...
\paragraph{Factor-wise paired differences.} Table~\ref{tab:ablation} reports the
best configuration of each variant and Table~\ref{tab:ablation-ci} the
factor-wise paired differences. On ImageNet-100, OPB-FS exceeds CEB-energy by
$2.67$ points (CI $[2.45, 2.92]$), the IB contrast is $+1.89$
(GPB $-$ NCM-ortho,
...
where CEB-energy is $1.19$ points below CEB ($[-1.50, -0.96]$): the sign of the
restricted-readout contrast depends on the accompanying geometry. Most informative is
the negative result on the prototype readout alone: NCM-learn trails the
plain backbone by $0.96$ points ($[-1.30, -0.69]$), so a nearest-class-mean
readout alone does not reproduce the GPB result. On MNIST the same
...
Section~\ref{sec:sweep}. Among the paired ImageNet-100 contrasts, the geometry
contrast is largest, followed by the IB and readout contrasts. These interventions
change the fitted training problem and therefore support factor-wise attribution, not
a unique causal decomposition of the benchmark margin.
...
Housing (MLP) against $\beta$: OPB with $a = 12$ remains on the plateau out to
...
$\beta = 5{\cdot}10^{-5}$, and the larger-anchor trajectories remain in a
stable $\E[\KL]$ band.}
...
\emph{(i) Collapse occurs for the free reference and the smallest anchor.} On
ImageNet-100, CEB's
...
$\beta = 25$. Orthogonality alone therefore does not prevent collapse: the
...
accompanied by collapse at strong compression. \emph{(ii) The observed collapse
point varies with anchor scale, and GPB matches or exceeds CEB at the reported
matched-compression points.} On ImageNet-100, OPB ($a = 12$) attains $84.2\%$ at $\beta = 1$
...
and negligible $\beta$ the off-axis variance is unconstrained. In the reported
strong-compression regime, the GPB trajectories retain substantially higher task
metrics while CEB reaches chance or near-zero $R^2$.
...
$a$ itself; across the measured grid, the location of the performance plateau varies
with this validation-selected hyperparameter. The KL decomposition shows that OPB's $\E[\KL]$ is
...
shrinking both terms toward zero; the constructed reference excludes the corresponding
zero-margin reference configuration
...
verify the comparator bound. Posterior separation is constrained by the
mismatch-conditioned statement (Proposition~\ref{prop:pgib-separation}) and
the fixed-scale condition of
Proposition~\ref{prop:opb-sweep}; consistent with that proposition's scope, we do
not claim that the map $\beta \mapsto$ accuracy is monotone; the larger-scale
configurations simply maintain a plateau over a wider part of this observed sweep.
...
\emph{(ii) The estimated plateau lies near the task-information level.} OPB
...
$\beta = 5$: $0.09$). These estimator-level values lie near $H(Y)$; because the
information-plane quantities are lower-bound estimates, they do not establish that
the representation contains exactly the task-relevant information and nothing else.

\emph{(iii) Matched-compression performance.}
...
$98.1\%$). These are paired empirical comparisons at two estimated compression
levels, not a causal attribution to geometry.
...
% =====================================================================
\subsection{Structural diagnostics: posterior geometry and tied prediction}
\label{sec:mechanism}
% =====================================================================

The main sweep compares predictive performance. This section separately reports
structural diagnostics on two representative settings, MNIST (MLP, classification)
...
\paragraph{The posterior.} Table~\ref{tab:mechanism} shows that the fitted posterior
has geometry close to the declared reference. On classification the posterior class centers are
...
Housing (one representative run per model). The EPB posterior lies near the prior line
$\rho\tilde y$, shrunk toward $\rho\,\mathbb{E}[\tilde y \mid x]$ by the
encoder's uncertainty.}
\label{fig:mechanism-axis}
\end{figure*}

\paragraph{Tied-readout performance.} Both heads of
...
CEB's $0.491$ on Housing. At this strong-compression setting, these values are
consistent with the main-table GPB column
(Section~\ref{sec:main-results}) and with the nearest-anchor analysis of the
appendix. The same fitted models exhibit posterior geometry close to the reference
and non-trivial prediction under a zero-parameter readout. Their co-occurrence is a
structural consistency diagnostic, not a causal attribution or a verification of the
...
a failure of practical informativeness: with $a=1$ the posterior centers collapse at
$\beta=25$ (mean $S_{ij}=0$), and the mismatch consumes the entire declared margin.
Equivalently, no pair satisfies the empirical counterpart of
Eq.~\eqref{eq:separation-tightness-ratio}. At the other end, $a=6$ overshoots the frame at
$\beta=5\cdot10^{-5}$ ($S/(\sqrt2a)=1.39$); this ratio falls to $0.51$ at
$\beta=25$ as compression pulls the posterior inward. Two scale-ordering
observations sharpen this: at $\beta=5\cdot10^{-5}$ the mismatch is largest for the
smallest frame ($349.8$ at $a=1$, against $82.1$ at $a=12$ and $62.3$ at $a=6$), and
the ordering reverses at $\beta=25$ ($0.56$ at $a=12$ against $2.54$ at $a=6$; the
$0.385$ at $a=1$ reflects the collapse above). Since the anchor-energy readout attains
its prediction optimum exactly at the anchors, a larger declared scale exerts a
stronger pull, making the tied readout self-enforcing. Every applicable
...
$0.786$ to $0.767$. This separates the mismatch diagnostic from the performance
...
\includegraphics[width=0.24\linewidth]{figures/fig_mismatch_c_housing_d_beta.png}\hfill
\includegraphics[width=0.24\linewidth]{figures/fig_mismatch_d_housing_r2_vs_d.png}
...
a discrete declared margin; at the tested larger scales, the bound is positive for
most class pairs, whereas it is positive for none at $a=1$. The regression posterior
is closer to the declared direction than to its scale, and its finite-bin lower bound
is often vacuous. Neither
