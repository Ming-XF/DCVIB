...
arbitrary label noise and model misspecification. Its residual floor algebraically separates
...
this optimization bound, the observed anchor mismatch yields a deterministic lower
bound on posterior separation. At the strongest reported compression, this bound is
positive for $96.9\%$, $62.3\%$, and $0\%$ of ImageNet-100 class pairs at anchor
scales $a=12,6,1$, respectively, showing that its informativeness is conditional on
the selected scale and realized mismatch; the regression bound is consistent but
often vacuous.
...
and has a higher mean than CEB in all twelve settings. These predictive comparisons
are reported separately from the conditional theoretical statements.
\end{abstract}
...
covariance mismatch. This decomposition identifies the sources of a non-zero floor,
but does not make its numerical value identifiable on a single-label benchmark; a
specified comparator's total KL can instead be estimated on held-out data. The bound
is informative for a target mismatch only when the comparator KL lies below that
target. The familiar
...
realizability premise, the realized mismatch gives a lower bound on posterior
separation (Proposition~\ref{prop:pgib-separation}). The latter bound is informative
only when the mismatch radii are smaller than the declared prior margin, a condition
that depends on anchor scale. None of these statements guarantees task accuracy or
selects that scale; they characterize the certificate and a mismatch-conditioned
geometric constraint.

\textbf{Experiments.} We separate predictive performance from structural diagnostics.
The twelve real-data settings compare utility: GPB is best or
tied-best in $7$ of $12$ settings and has a higher mean than CEB in all $12$, but these comparisons
are not presented as verification of an alignment rate. The structural analysis instead
reports posterior geometry and tied-head prediction without assigning a causal effect.
Direct mismatch measurements evaluate the separation inequality quantitatively: at
$\beta=25$ its positive-pair fraction on ImageNet-100 is $96.9\%$, $62.3\%$, and
$0\%$ for $a=12,6,1$, whereas the EPB posterior follows the regression axis more
closely in direction than in scale. A
...
$\beta$ sweep. On a real dataset, a specified feasible comparator's total KL can be
estimated on held-out data, but the best comparator and its ambiguity--approximation
decomposition are not identified by a single-label benchmark.
...
(4) unconditional certificate preservation and mismatch-conditioned posterior separation
