...
identified in prior work is one manifestation of this freedom; a complementary issue
persists even when labels are noisy or the information-plane knob moves: the
...
We introduce the geometric prior bottleneck (GPB), which keeps CEB's stochastic
posterior and single conditional KL while restricting the reference to a qualified,
bounded, stateless family. For classification, the orthogonal prior bottleneck (OPB)
maps the class table to an equal-radius orthogonal frame and uses the corresponding
Gaussian anchor energies as the classifier. For regression, the equidistant prior
bottleneck (EPB) maps standardized labels to an isometric axis and uses the same axis
for prediction. Thus each prior mean has two simultaneous roles: it is a target in the
KL and an anchor in the readout. The construction is computed from the current
all-label prior table, with no EMA, prototype memory, or batch-dependent geometry.

GPB reuses geometric primitives that appear in prototype, ETF, orthogonal-classifier,
and fixed-frame methods, but changes the object to which they are attached. OPB does
not introduce a new ETF or prototype objective: its frame is the mean map of a
conditional Gaussian prior, and its anchor-energy rule is the Bayes readout induced by
the shared covariance. EPB similarly uses distance preservation as a prior mean map,
not as a separate contrastive loss. This object-level distinction, together with the
relationship to existing geometric classifiers, is summarized in the boundary table
of Appendix~\ref{sec:related}.
...
mismatch (Appendix~\ref{app:deferred-theory}). Independently, the mismatch-conditioned
separation proposition converts the realized distance to the declared prior anchors
into a lower bound on posterior-center separation. Together these results connect the
