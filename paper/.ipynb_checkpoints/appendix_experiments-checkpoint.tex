...
$82$--$86\%$ MC-10 accuracy (averaged over ten posterior samples) while
spreading across the full log-$\widehat D$ axis: accuracy is insensitive to the
mismatch, and the strongly aligned right end of the band sits no lower than the
rest, so deep compression costs nothing on the task metric. The slack between
the bound and the realized separation (both defined in
Section~\ref{sec:direct-mismatch}) stays non-negative at every reported point
(b): the bound is consistent but conservative.
...
accuracy against $\widehat D$ (color $=\beta$, marker $=a$). (b) Slack percentiles
(p05--p50--p95). (c) Cal.\ Housing anchor
mismatch against $\beta$ by isometric scale $\rho$, decomposed into axial and
off-axis contributions. (d) Cal.\ Housing bin-pair separation against
...
imply a comparable task-metric gain. Panel (d) of Fig.~\ref{fig:direct-mismatch-deferred}
...
positive bound. Panel (c) of Fig.~\ref{fig:direct-mismatch-deferred} shows these
mismatch--$\beta$ curves with their axial/off-axis decomposition: the dotted
off-axis contribution stays below $0.01$ nats, so the posterior lies essentially
