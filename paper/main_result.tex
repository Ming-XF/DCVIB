...
\begin{table*}[t]
\centering
\caption{Test accuracy (\%) on classification tasks and test $R^2$ on regression tasks (mean over runs). Each entry reports the best configuration over the tuned $\beta$ (and anchor-scale) grid; the best entry per dataset is bolded.}
...
\hline
Dataset & Backbone & Base & VIB & SVIB & NIB & CEB & DVCCA & GPB & GPB-L \\
\hline
