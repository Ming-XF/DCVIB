% fgib_theory.tex —— FGIB 对 SIB（Kolchinsky et al., ICLR 2019 "Caveats"）确定性退化问题的规避性推导
% 记号沿用 Kolchinsky et al. (2019) 与 Fischer (2020)；用法同 dcvib_theory.tex
...
with linear mean head $\mu(x) = A\,h(x)$, and the label-conditional prior is a \emph{fixed}
...
Following Kolchinsky et al.\ (2019), we write the IB curve $F(r)$ (their Eq.~1), the IB
...
By the linear-invariance lemma for Gaussian KL (dcvib\_theory.tex, Lemma~1), the invertible
map $\tilde{z} = A^{-1}z$ reparameterizes the side path to $\mu_{\tilde q}(x) = h(x)$ with
unchanged KL, so we take $\mu(x) = h(x)$ without loss of generality.
...
\item \emph{FGIB}: the compression term is $\operatorname{KL}(q_y \| r_y) \supseteq
\tfrac{1}{2}\|\mu_y - a v_y\|^2 = \tfrac{1}{2}(s-a)^2$ --- \emph{quadratic} in the
alignment scalar $s$, hence strictly convex, because the KL is measured against a
...
$\|\mu_p(y)\| \approx a$, so $\operatorname{KL} \supseteq \tfrac{1}{2}\|\mu - \mu_p(y)\|^2$
is again quadratic in $\mu$ for the fixed anchor --- the same convexity mechanism carries
over. (iv)~Training starts at $\operatorname{KL} \approx a^2/2$ (zero-initialized logvar
