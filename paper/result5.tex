...
\hline
\end{tabular}
\caption{Controlled label-noise floor (mean $\pm$ std over five seeds). The nine-point $\beta$ grid spans $10^{-3}$ to $10$; max excess is the largest mean excess over the grid; the last column is the fitted population objective minus the explicit comparator objective at $\beta=10$ (negative means the fit improves on the comparator).}
