...
\caption{Controlled label-noise floor (mean $\pm$ std over seeds); max excess is $\max_\beta(\widehat D-\delta_{\rm noise})$, and the last column is the fitted objective minus the explicit comparator objective at $\beta=10$.}
\label{tab:controlled-noisy-alignment}
\begin{tabular}{c c c c c c}
\hline
$r_{\rm n}$ & $\delta_{\rm noise}$ & $\widehat D(\beta{=}10^{-3})$ & $\widehat D(\beta{=}10)$ & max excess & $\widehat L-L_{\rm comp}$ \\
\hline
