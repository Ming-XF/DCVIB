...
$h = f_{\mathrm{enc}}(X)$; the mean head and the logvar head parameterize the learned
diagonal posterior $q(z|x) = \mathcal{N}(\mu(x), \operatorname{diag}\sigma^2(x))$; the code $z$ is sampled from
...
$r(z|y) = \mathcal{N}(a\,v_y, \tau^2 I)$ with fixed \emph{anchor variance}
$\tau^2=1$: the frame $V = [v_1, \dots, v_K]$ is the orthonormal factor of a
...
logvar head fixes $\sigma^2 = 1$ at initialization, which equals the fixed prior
variance $\tau^2 = 1$, so training starts at
...
\label{eq:opb-prior}
\end{equation}
where $a \ge 0$ is the anchor scale and the shared prior variance is fixed at
$\tau^2=1$.
