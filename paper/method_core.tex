...
\end{equation}
whose means obey a declared two-sided separation for discrete labels,
\begin{equation}
\Delta\le\|m_\theta(y)-m_\theta(y')\|\le\bar\Delta,
\label{eq:pgib-separation-condition}
\end{equation}
or a two-sided label metric condition
\begin{equation}
...
\label{eq:opb-qr}
\end{equation}
and we assume the monitored matrix has full column rank:
...
whose scale is fixed by the prior (Proposition~\ref{prop:opb-energy}). For regression, EPB uses
\begin{equation}
p_\theta^{\rm EPB}(z|y)=\mathcal N(\rho\tilde y\,u,\tau^2I),qquad
\|\mu_p(y)-\mu_p(y')\|=\rho|\tilde y-\tilde y'|,
\label{eq:epb-prior}
\end{equation}
and the tied projection $\hat{\tilde y}=u^\top z/\rho$. Detailed polar construction,
initialization, covariance relaxation, and the relation to prototype/ETF methods are
given in Appendices~\ref{app:construction} and~\ref{sec:related}.

For OPB, the general objective is equivalently
\begin{equation}
...
\end{equation}
so the same stateless anchors enter both the KL and the tied readout.
...
certificate identity; batch-dependent references would not satisfy this statement.
\end{proposition}
