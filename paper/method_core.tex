...
For discrete labels, let $m_k=\E[\mu_\phi(X)\mid Y=k]$ and
\begin{equation}
...
\end{equation}
For continuous labels, define $m(y)=\E[\mu_\phi(X)\mid Y=y]$ and
\begin{equation}
...
\begin{corollary}[Beta-dependent geometry transfer]
\label{cor:pgib-beta-transfer}
Fix the prior family, a feasible comparator $w_0$, and an $\varepsilon$-optimal
solution $\widehat w_\beta$ for each $\beta$, with $\varepsilon$ uniformly bounded.
Define
\begin{equation}
B_\beta:=\mathcal K(w_0)+\frac{\mathcal R(w_0)+\varepsilon}{\beta}.
...
$S_{\beta,\xi}$ with $P_Y(S_{\beta,\xi})\ge 1-\xi$ such that, for almost every
$y,y'\in S_{\beta,\xi}$,
\begin{equation}
\|m(y)-m(y')\|
\ge \rho_{\min}d_Y(y,y')
...
\end{equation}
If the comparator KL is zero and the remaining quantities are bounded while
$\beta$ varies, the certified separation deficit is $O(\beta^{-1/2})$; with a
non-zero comparator KL, the bound approaches a comparator-dependent floor.
\end{corollary}
