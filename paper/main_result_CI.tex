...
\begin{table*}[t]
\centering
\caption{Paired-difference 95\% confidence intervals of GPB over CEB and DVCCA (GPB minus baseline, per setting; classification in percentage points, regression in $R^2$). Bold/† as in the baselines table.}
...
\begin{tabular}{llcc}
\hline
Dataset & Backbone & CEB & DVCCA \\
\hline
MNIST & MLP & \textbf{+0.1 [+0.0, +0.2]} & \textbf{+0.3 [+0.2, +0.4]} \\
MNIST & CNN & +0.0 [-0.0, +0.1]$^{\dagger}$ & +0.0 [-0.1, +0.1]$^{\dagger}$ \\
ImageNet-100 & MLP & \textbf{+1.6 [+1.5, +1.7]} & \textbf{+1.4 [+1.1, +1.7]} \\
ImageNet-100 & CNN & \textbf{+3.1 [+2.4, +3.7]} & \textbf{+2.9 [+2.4, +3.5]} \\
Cora & GCN & \textbf{+1.8 [+1.5, +2.3]} & \textbf{+1.2 [+0.4, +1.9]} \\
IMDb & RNN & \textbf{+1.3 [+0.3, +2.1]} & \textbf{+1.1 [+0.8, +1.5]} \\
AG News & RNN & +0.0 [-0.1, +0.2]$^{\dagger}$ & +0.1 [-0.0, +0.2]$^{\dagger}$ \\
\hline\hline
Cal. Housing & MLP & +0.000 [-0.003, +0.004]$^{\dagger}$ & +0.002 [-0.005, +0.009] \\
STS-B & RNN & +0.009 [-0.009, +0.025] & +0.005 [-0.020, +0.026] \\
ZINC & GCN & \textbf{+0.009 [+0.005, +0.011]} & \textbf{+0.005 [+0.002, +0.007]} \\
AgeDB & MLP & \textbf{+0.022 [+0.006, +0.038]} & \textbf{+0.036 [+0.012, +0.057]} \\
AgeDB & CNN & \textbf{+0.082 [+0.064, +0.099]} & \textbf{+0.149 [+0.065, +0.310]} \\
\hline
 &  & 8*/3† & 8*/2† \\
\hline
\end{tabular}
...
\begin{table*}[t]
\centering
\caption{Paired-difference 95\% confidence intervals of GPB over the external non-IB baselines NCBD (classification settings only) and AdaCap (regression settings only); GPB minus baseline, classification in percentage points, regression in $R^2$. Bold/† as in the baselines table.}
...
\hline
MNIST & MLP & \textbf{+0.6 [+0.4, +0.8]} & -- \\
MNIST & CNN & \textbf{+0.1 [+0.1, +0.2]} & -- \\
ImageNet-100 & MLP & \textbf{+0.8 [+0.6, +0.9]} & -- \\
ImageNet-100 & CNN & \textbf{+5.5 [+4.3, +6.9]} & -- \\
Cora & GCN & \textbf{+0.8 [+0.0, +1.9]} & -- \\
IMDb & RNN & +0.2 [-0.5, +0.7] & -- \\
AG News & RNN & \textbf{+0.5 [+0.2, +0.8]} & -- \\
\hline\hline
Cal. Housing & MLP & -- & \textbf{+0.014 [+0.009, +0.019]} \\
STS-B & RNN & -- & \textbf{+0.115 [+0.086, +0.135]} \\
