...
hidden sizes $512$--$256$, dropout $0.2$, batch
size $128$, and a $100$-epoch budget with early stopping (patience
$10$); the learning rate is tuned on the shared six-point grid
$\{6\times10^{-4},8\times10^{-4},10^{-3},2\times10^{-3},4\times10^{-3},6\times10^{-3}\}$,
identical for every method and dataset; dataset-specific defaults are
likewise shared by every method on
that dataset: Cora uses batch size $512$,
...
NCBD and AdaCap; GPB is OPB on classification rows and EPB on regression rows,
with GPB-L denoting the identical prior and a free head. The main
comparison equalizes the tuning budget across methods: GPB and GPB-L are
swept over a two-dimensional $\beta\times$scale grid, the six-point
$\beta$ grid times a six-point anchor-scale grid ($a$ for
classification, $\rho$ for regression); every other $\beta$-weighted
method, VIB, SVIB, NIB, CEB, and DVCCA, is swept over the same six-point
$\beta$ grid times a six-point learning-rate grid; NCBD and AdaCap are
swept over their own six-value grids, the temperature $\tau$ and the
Tikhonov $\lambda$, times the same learning-rate grid. Every method with
a method-specific hyperparameter thus evaluates the same number of
configurations ($36$), so no method receives a larger search budget
than the others; the plain backbone has none. For each method and setting, the
...
same selection rule for every method and every ablation variant, which
share the same two-dimensional grids
...
\hline
Base & --- & $1$ \\
VIB, SVIB, NIB, CEB, DVCCA & $\beta\in\{10^{-4},10^{-3},10^{-2},10^{-1},1,10\}$ $\times$ the lr grid & $36$ \\
NCBD & temperature $\tau\in\{0.05,0.1,0.2,0.3,0.5,1\}$ $\times$ the lr grid & $36$ \\
AdaCap & Tikhonov $\lambda\in\{10^{-2},10^{-1},1,10,10^{2},10^{3}\}$ $\times$ the lr grid & $36$ \\
GPB (OPB / EPB) & the same $\beta$ grid $\times$ $a=\rho\in\{1,2,4,6,8,12\}$ & $36$ \\
\hline
\end{tabular}
...
best mean validation metric is selected per method and setting. The lr
grid is the shared six-point learning-rate grid given above, identical
for every method and dataset. GPB sweeps the anchor scale $a$ (resp.\
isometric scale $\rho$) jointly with $\beta$, and every other method
with a method-specific hyperparameter sweeps the learning rate jointly
with it, so that every such method evaluates the same number of
configurations ($36$).}
...
read on the sampled posterior, and the residual is the stochastic-path difference
$I(X;Z)-I(Y;Z)$. The information plane is informative: CEB
...
$\beta$ grid (log $x$-scale in (a)), lines connect $\beta$ in
ascending order. CEB
