...
\emph{Scope of the comparison argument.} Proposition~\ref{prop:pgib-alignment} applies to
...
\begin{proof}[Proof of Proposition~\ref{prop:pgib-alignment}]
Since $\Sigma_\theta(y)\preceq\tau^2_{\max}I_d$ is equivalent to
$\Sigma_\theta^{-1}(y)\succeq\tau_{\max}^{-2}I_d$, the Gaussian KL contains a mean
term
$\frac12(\mu_\phi(x)-m_\theta(y))^\top\Sigma_\theta^{-1}(y)(\mu_\phi(x)-m_\theta(y))
\ge\|\mu_\phi(x)-m_\theta(y)\|^2/(2\tau^2_{\max})$. Hence
...
For (a), Jensen's inequality gives
$\|m_k - m_\theta(k)\| \le \sqrt{2\tau^2_{\max} D_k}=r_k$
(the norm is convex, and $\E\|\mu_\phi(X)\|^2<\infty$ makes $m_k$ well defined), and
the triangle inequality on
$m_i - m_j = (m_i - m_\theta(i)) + (m_\theta(i) - m_\theta(j))
+ (m_\theta(j) - m_j)$ gives the two-level bound
$\|m_i-m_j\|\ge\|m_\theta(i)-m_\theta(j)\|-r_i-r_j\ge\Delta-r_i-r_j$, the last step
by Eq.~\eqref{eq:pgib-separation-condition}. The first level is positive exactly
when $\Gamma_{ij}<1$ and the declared-margin level exactly when
$r_i+r_j<\Delta$; for OPB the anchor pair distance is the constant $\sqrt2a$ of
Eq.~\eqref{eq:opb-prior}, so the declared margin satisfies $\Delta=\sqrt2a$ and
the two conditions coincide. With equal mismatch $D_i=D_j=D$, the common condition
$\Gamma_{ij}=2\sqrt{2\tau^2_{\max}D}/(\sqrt2a)<1$ reduces to
$D<a^2/(4\tau^2_{\max})$. For (b), the same two inequalities hold pointwise for
$P_Y$-a.e.\ $y$: Jensen on the regular conditional expectation gives
$\|m(y) - m_\theta(y)\| \le \sqrt{2\tau^2_{\max} D(y)}=r(y)$ (the norm is convex),
and the triangle inequality with Eq.~\eqref{eq:pgib-lipschitz} gives
Eq.~\eqref{eq:pgib-separation-continuous}; intersecting the two measure-one sets of
valid $y$'s yields the $P_Y\otimes P_Y$-almost-everywhere pair statement.
\end{proof}
...
Proposition~\ref{prop:pgib-alignment} gives
$\mathcal D(\widehat w_\beta)\le B_\beta$. For discrete labels, the tower property
gives $\mathcal D(\widehat w_\beta)=\sum_k\pi_kD_k$, so
$D_k\le B_\beta/\pi_k\le B_\beta/\pi_{\min}$. Substitution into
Theorem~\ref{thm:pgib-separation}(a) gives Eq.~\eqref{eq:pgib-beta-separation}.
For continuous labels, Markov's inequality on the nonnegative random variable
$D(Y)$ gives $P_Y(D(Y)>B_\beta/\xi)\le\xi$; on
$S_{\beta,\xi}=\{y:D(y)\le B_\beta/\xi\}$, Theorem~\ref{thm:pgib-separation}(b)
gives Eq.~\eqref{eq:pgib-beta-separation-continuous}. The rate statement follows
...
log-variance clamped to a fixed interval. It is therefore a covariance-relaxed member of the geometric prior family
$\mathcal G$: the OPB/EPB mean geometry and readout scale remain fixed, while
