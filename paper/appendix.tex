...
their mechanism analysis evaluates realized mismatch and geometry. The controlled
...
Proposition~\ref{prop:pgib-separation}(a) gives
Eq.~\eqref{eq:pgib-alignment-separation}. For (b), Markov's inequality gives
$P_Y(D(Y)>t)\le B_0/t$; on $S_t$ the pointwise transfer of
Proposition~\ref{prop:pgib-separation}(b) applies with $D(y),D(y')\le t$.
\end{proof}
...
The population-weighted experiment of Section~\ref{sec:controlled-validation} uses a
fixed frame and the standard trainable posterior variance. Figure~\ref{fig:noisy-alignment-sensitivity}
changes these choices one at a time. Fixing the posterior variance to
...
\begin{figure*}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig_noisy_alignment_appendix.png}
...
This section gives the finite-sample quantities behind
Section~\ref{sec:direct-mismatch}. All statistics are first computed within each seed
and then aggregated across the five seeds; class or bin pairs are not treated as
independent replicates. Every checkpoint in both requested grids is present, and no
collapse or variance-explosion point is removed.

\paragraph{Classification.} For test examples of class $k$, define
\begin{equation}
...
\le\sqrt{2\widehat\tau^2_{\max}\widehat D_k}$, and the triangle inequality gives
Eq.~\eqref{eq:empirical-class-separation-bound}. Both are evaluated without clipping
the lower bound. Across all OPB scales, $\beta$ values, seeds, and classes, no Jensen
or separation-slack violation exceeds $10^{-6}$. Figure~\ref{fig:direct-mismatch-class}
shows how the actual separation, raw lower bound, fraction of informative pairs, and
slack change over the sweep. CEB's mismatch is shown descriptively, but its bound and
slack entries are left empty because its reference has no declared $\Delta$.

\begin{figure*}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig_mismatch_appendix_cls.png}
\caption{ImageNet-100 class-separation diagnostics. (a) Mean posterior-center
distance and the raw (unclipped) mismatch-dependent lower bound against $\beta$;
CEB mismatch is included only as a descriptive reference. (b) Fraction of OPB class
pairs with a positive lower bound. (c) The $5$th, $50$th, and $95$th percentiles of
the separation slack. Statistics are computed within seed before aggregation.}
\label{fig:direct-mismatch-class}
\end{figure*}

\paragraph{Regression.} We partition the standardized test labels into $20$
equal-width bins and merge any bin containing fewer than $30$ examples. For bin
$B_b$, let $\bar y_b$ be its mean label, $\widehat m_b$ its posterior center,
$\widehat D_b$ its average sample-to-own-anchor mismatch, and
$h_b:=\operatorname{diam}(B_b)$. The finite-bin lower bound used in the diagnostic is
\begin{align}
...
\end{align}
The last term is the quantization correction from
Remark~\ref{rem:pgib-binning}. Figure~\ref{fig:direct-mismatch-regression} plots the
actual bin-center distances against the declared isometric distances for four
$\beta$ values at $\rho=6$. The points lie systematically below the identity line,
consistent with the measured normalized axial slope of $0.73$--$0.77$. After the
quantization correction, $43\%$ of bin pairs have a positive bound; all raw slacks
remain non-negative to numerical tolerance.

\begin{figure*}[t]
\centering
\includegraphics[width=\linewidth]{figures/fig_mismatch_appendix_reg.png}
\caption{Cal.\ Housing finite-bin regression diagnostic at $\rho=6$. Each panel
compares posterior bin-center distance with the declared prior distance
$\rho|\bar y_b-\bar y_c|$ at one $\beta$; the identity line denotes exact scale
transfer. The mismatch-dependent bound additionally includes the two mismatch terms
and the bin-diameter correction of
Eq.~\eqref{eq:empirical-regression-separation-bound}.}
\label{fig:direct-mismatch-regression}
\end{figure*}
