...
\subsection{Hyperparameter sensitivity tables}
\label{app:hparam-tables}

Table~\ref{tab:compression} reports the full $\beta$ grid on the two sweep settings
(the hyperparameter-sensitivity view; the main-text curves report the same data on
the realized-$\E[\KL]$ axis), and Table~\ref{tab:compression_decomp} reports the
test-set decomposition of every sweep configuration: the prediction loss (CE or
MSE), the mean-mismatch and variance-mismatch terms of the KL, the total
$\E[\KL]$, and the task metric.
