...
$I(X;T_\alpha|Y) = 0$. By the certificate identity in Eq.~\eqref{eq:gap}, with the true
backward encoder the certificate is tight, $\mathrm{ReX} = I(X;T_\alpha|Y) = 0$, and the optimal
...
Theorem~\ref{prop:pgib-alignment} gives $\mathcal D(\widehat w)\le B_\beta$.
For discrete labels, $\mathcal D(\widehat w)=\sum_k\pi_kD_k$, so
...
For continuous labels, Markov's inequality gives
$P_Y(D(Y)>B_\beta/\xi)\le\xi$; on
...
directly from the definition of $B_\beta$.
\end{proof}
...
bound tighter than CEB's: Eq.~\eqref{eq:gap} is the usual CEB gap
