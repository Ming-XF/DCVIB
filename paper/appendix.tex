...
every $\beta$-weighted method --- VIB, SVIB, NIB, CEB, DVCCA, and GPB
--- is swept over the same $\beta$ grid (six configurations) and
selected by the same rule, so no method receives a larger search budget
than the others; GPB's anchor scale $a$ (resp.\ isometric scale $\rho$)
is fixed at $12$ throughout the main comparison; NCBD and AdaCap are
swept over their own six-value grids. For each method and setting, the
...
GPB (OPB / EPB) & $\beta\in\{10^{-4},10^{-3},10^{-2},10^{-1},1,10\}$; $a=\rho=12$ fixed & $6$ \\
\hline
\end{tabular}
...
best mean validation metric is selected per method and setting. GPB's anchor
scale is fixed at $a=\rho=12$; scale variation is reported as
