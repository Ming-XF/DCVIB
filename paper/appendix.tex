...
gives $\mathcal D(\widehat w_\beta)=\sum_k\pi_kD(k)$, so
$D(k)\le B_\beta/\pi_k\le B_\beta/\pi_{\min}$. Substitution into
Theorem~\ref{thm:pgib-separation} gives Eq.~\eqref{eq:pgib-beta-separation}.
For continuous labels, Markov's inequality on the nonnegative random variable
$D(Y)$ gives $P_Y(D(Y)>B_\beta/\xi)\le\xi$; on
$S_{\beta,\xi}=\{y:D(y)\le B_\beta/\xi\}$, Theorem~\ref{thm:pgib-separation}
