...
simultaneously as Gaussian-prior means, KL targets, and readout anchors. The
orthogonal frame is one admissible instance of the declared geometry rather than a
commitment of the framework: any anchor set satisfying the two-sided separation of
Eq.~\eqref{eq:pgib-separation-condition}, including the centered simplex, enters
the same KL objective with identical guarantees. We adopt the orthogonal convention
for its stable rank-$K$ polar construction with a full-rank identity initialization
(Appendix~\ref{app:construction}) and the established record of equidistant class
geometries in classification \citep{deng2014equidistant,xu2022orthogonal}.
