...
regression rows. The main comparison equalizes the search budget across methods: the plain
backbone has no method-specific hyperparameters; VIB, SVIB, NIB, CEB, and
DVCCA are swept over the same 30-point $\beta$ grid (six decades, five
values per decade); NCBD and AdaCap use 30-value grids of their own; and
GPB searches a two-dimensional grid of $6\times5=30$ configurations,
$\beta\in\{10^{-4},10^{-3},10^{-2},10^{-1},1,10\}$ (one value per decade)
times the anchor scale $a$ (resp.\ isometric scale $\rho$) on the
five-value grid $\{1,3,6,9,12\}$. The diagnostics in
Secs.~\ref{sec:direct-mismatch}, \ref{sec:robustness}, \ref{sec:sweep},
and \ref{sec:info-plane} read trained configurations on extended grids
beyond the selection grid ($\beta$ up to $25$, anchor scales
$\{1,6,12\}$) and play no role in model selection. For each method and setting,
...
paired by seed. Each row of Table~\ref{tab:ablation-ci} pairs two variants that
differ in the factor named in its first column and reports the paired difference
with its bootstrap CI together with the variant's mean; the variant definitions
are in Appendix~\ref{app:ablation}.

\input{ablation_result_ci}

The table shows the ablation results, in which two factors dominate. The
orthogonal frame fixes all pairwise anchor distances and the posterior inherits
this structure, so removing the geometric construction costs by far the most.
The conditional compression is no passenger on that geometry: removing the
stochastic bottleneck and its KL while keeping the frame and readout still
loses accuracy on its own. Freezing the prior frame removes the co-adaptation
on which the conditional KL relies, at real cost to accuracy. A free head can
refit the boundaries independently of the frame and undo part of the geometry,
while the tied readout keeps prediction on it. Only the variance switch is a
null, so the fixed-covariance reference is empirically benign.

\subsection{Compression sweeps}
\label{sec:sweep}
This subsection shows the classification sweep on ImageNet-100 (MLP):
test accuracy and realized conditional KL against the extended $\beta$
grid from $5\cdot10^{-5}$ to $25$, for CEB and OPB at anchor
...
The complementary scale-axis view, reading the full nine-value scale
grid $\{1,2,4,6,8,10,12,14,16\}$ at three fixed $\beta$ levels, is reported in
