...
STS-B the plain backbone remains best). The tied head never hurts: GPB is within
$0.4$ points of the free-head variant GPB-L in $11/12$ settings, and $1.20$ points
better on ImageNet-100 (CNN). Among the external non-IB baselines, AdaCap is
...
$11.2$ nats as $r$ runs over the noise grid. The explicit comparator takes
...
and $\widehat m_k=\frac1{n_k}\sum_{n:y_n=k}\mu_\phi(x_n)$, the sample counterparts
of $D(y)$ and $m(y)$ in Theorem~\ref{thm:pgib-separation}
($\tau_{\max}=\tau=1$). Its two steps read, at sample level, as the per-class
Jensen check $\|\widehat m_k-aq_{\theta,k}\|\le\sqrt{2\tau^2\widehat D_k}$ and the
...
paired intervention ladder that switches four design factors independently, namely
the prior geometry, the anchor scale, the readout, and the IB term itself, plus two
...
Cal.~Housing, all with MLP backbones; Table~\ref{tab:ablation-ci}) at its best
...
On ImageNet-100, GPB reaches $84.2$ points, within $0.1$ of the free-scale OPB-FS
and well above CEB ($82.8$); on MNIST all variants tie within $0.4$ points, and on
Housing every EPB variant ties within $0.01$ $R^2$, consistent with the flat
strong-compression plateaus of Section~\ref{sec:sweep} (means in
Table~\ref{tab:ablation}, Appendix~\ref{app:ablation}). The paired contrasts
(Table~\ref{tab:ablation-ci}; accuracy points on ImageNet-100) rank
the factors: geometry is the largest ($+2.67$,
