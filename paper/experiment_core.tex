...
the prior geometry, the readout, the IB term itself, and the trainability of the
prior frame, plus two auxiliary contrasts on the prototype readout itself. Every
variant is trained on the two mechanism settings (MNIST and ImageNet-100, both
with MLP backbones) at its best
...
the factors on ImageNet-100: geometry is the largest ($+2.63$,
GPB over CEB-energy), followed by the IB term ($+1.89$, GPB over NCM-ortho, which
additionally contains frame trainability) and the tied readout ($+0.82$, GPB over
GPB-L). Two patterns matter as much as the ranking. First, the readout contrast
...
(Table~\ref{tab:ablation-ci}, frame trainability). The prior encoder plays the
