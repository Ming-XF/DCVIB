...
item-wise ablations (Sec.~\ref{sec:ablation}) isolate the latter.
...
baselines NCBD (unit-hypersphere prototypes with a simplex-ETF
contrastive optimum) and AdaCap (contrastive loss with a closed-form
readout); GPB is OPB on classification rows and EPB on
...
30-point $\beta$ grid (six decades, five values per decade), and NCBD and
AdaCap use 30-value grids of their own. GPB's anchor scale $a$ (resp.\
isometric scale $\rho$) is fixed and selected on the five-value grid
$\{1,3,6,9,12\}$, so GPB searches a two-dimensional grid of $6\times5=30$
configurations, matching the baselines' budget. For each method and setting,
...
intervals are in Appendix~\ref{app:main-ci};
...
baseline) and is best or tied-best in $8/12$ settings. Across the $84$ per-setting paired comparisons (the six IB baselines on all $12$ settings, plus the two external baselines NCBD and AdaCap on their applicable classification and regression settings), GPB is significantly better in $57$ of them and significantly worse in only $2$ (STS-B versus the plain backbone; ZINC versus AdaCap); under a Holm--Bonferroni adjustment across the $84$ comparisons, the counts are $49$ and $2$. GPB is significantly better than every applicable baseline in $3/12$ settings and than a majority of baselines in $8/12$ (last column of Table~\ref{tab:main_result}). The gains are largest where the task has margin to
...
competitive on regression (best on ZINC). Across modalities, the same construction
...
\label{sec:ablation}
The main comparison changes several factors simultaneously, so we also report
item-wise ablations that switch five design factors independently, namely the
prior geometry, the readout, the prior variance, the trainability of the prior
frame, and the IB term itself. Every variant is trained on ImageNet-100 (MLP
backbone) at its best validation-selected configuration, and all comparisons are
paired by seed. GPB is held at its main-comparison configuration ($\beta=1$,
$a=14$); the readout contrast pairs the two variants at that same configuration,
and each remaining variant is reported at its own best validation-selected
configuration. Each row of Table~\ref{tab:ablation-ci} pairs two variants that
differ in the factor named in its first column and reports the paired difference
with its bootstrap CI together with both means; the variant definitions are in
...
the factors on ImageNet-100: geometry is the largest ($+2.81$,
GPB over CEB-energy), followed by the IB term ($+2.07$, GPB over NCM-ortho,
which additionally contains frame trainability), the frozen prior ($+1.02$,
GPB over OPB-RV), and the tied readout ($+0.96$, GPB over GPB-L), while fixing
the prior variance is a null ($+0.05$, CI spanning zero). The readout contrast
is read at the shared main-comparison configuration ($\beta=1$, $a=14$), where
the tied form gains about one point, so binding the readout to the declared
geometry is never a liability. The frozen-prior
ablation (Table~\ref{tab:ablation-ci}, prior freeze) shows the trainable prior
carries task benefit on classification: GPB exceeds the variant with all prior
parameters frozen (frame and variance) by $+1.02$ points. Since the variance
contrast shows that variance learnability alone is a null, this gain is
attributable to the frame co-adaptation. The prior encoder plays the
role of CEB's backward encoder: $r(z|y)$ models the representation distribution
under the label and co-adapts with the posterior $q(z|x)$, and freezing it removes
the co-adaptation on which the conditional KL relies
\citep{fischer2020conditional}. The freedom it exercises is a rotation, which the
posterior-mean head absorbs exactly (the isotropic-prior KL and the energy readout
are invariant), so the trainable-frame gain reflects the co-adaptation mechanism
at the optimization level rather than an increase in representational capacity.
The variance null completes the picture: at matched tuning budget, fixing
$\tau^2=1$ costs nothing, so the fixed-covariance reference of the method is
empirically benign. These interventions
change the fitted training problem and therefore support item-wise attribution,
...
The complementary scale-axis view, reading the full five-point scale
grid $\{1,3,6,9,12\}$ at three fixed $\beta$ levels, is reported in
...
are in Appendix~\ref{app:info-plane}, with the Housing companion there as well.
...
tighter information estimates, and alternative anchor geometries
(hierarchy-aware or similarity-preserving) are natural
