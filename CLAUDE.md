...
- MLP 骨干（`model/mlp/`；`model/__init__.py` 从 `.mlp` 重导出，`train.py` 的 `from model import ...` 接口不变）：
  ...
  - `FGIB`：固定几何信息瓶颈，同 DCVIB，但标签条件先验换成 `register_buffer` 的固定锚点先验表（不参与梯度）：`build_anchor_prior`（`model/mlp/utils.py`）对随机矩阵做 QR 分解得到 K 个正交方向，缩放 `anchor_scale` 作为各类先验均值、方差固定为 `anchor_var`（1.0）；KL 无法靠移动先验减小，只能靠 z 对齐锚点，正则力持续存在。`anchor_scale=0` 时各类锚点重合于原点，退化为 N(0,I) 先验。要求 `num_classes <= z_dim`（imagenet100 的 100 ≤ 256 满足）。仅分类任务（housing 直接报错）。
