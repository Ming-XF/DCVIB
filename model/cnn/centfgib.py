"""CentFGIB（center-loss FGIB，MSE-to-anchor FGIB）CNN 模型定义。

消融模型（审稿人要求）：固定锚点但直接使用 MSE/center loss 代替 KL，
与 MLP 版 model/mlp/centfgib.py 同构。旁路损失为 0.5·‖μ − a·v_y‖²
（逐样本求和后 batch 平均，与 KL 的位移项在 τ²=1 时同尺度），无
logvar 头参与、无方差项、无采样；分类器吃确定性 h（同 CNN 版 FGIB）。
"""
...
class CentFGIB(FGIB):
    """CNN 版 center-loss 消融：旁路项为直接 MSE-to-anchor，无 KL。"""
