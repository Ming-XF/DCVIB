...
class CEB(nn.Module):
    ...
    b(z|y) 用无激活的线性层把标签（分类 one-hot / 回归连续 y）映射为先验
    r(z|y) 的均值和对数方差，训练损失中加入 KL(q(z|x) || r(z|y))。

    图转导设置：KL 按 mask（训练时为 train_mask）只在训练节点上计算，
    标签条件先验只接触训练节点的标签（避免标签泄漏）。
    batch 非 None 时（图级任务）编码器后先做 mean 图读出，先验按图标签
    条件（continuous_y 时先验条件在缩放后的连续 y 上，同 mlp/rnn 版约定）。
    ...
    def __init__(
        ...
        # 标签先验线性层：无激活，标签 -> (mu_prior, logvar_prior)
    ...
    def forward(self, x, labels=None, stochastic=True, adj_norm=None, mask=None, batch=None):
        ...
        if self.continuous_y:
            y_feat = labels.float().unsqueeze(-1)  # 连续 y（缩放后）→ (B, 1)
        else:
