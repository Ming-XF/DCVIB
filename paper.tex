...
corner of the IB curve; its residual certificate is identically zero along a manifold
that ranges from chance level to a full copy of the label; the posterior and prior
variances race each other to the numerical clamp; and the certified object is the noisy
training code, not the representation that inference actually consumes. We propose the
Fixed-Geometry Information Bottleneck (FGIB), which keeps the inference path
deterministic and attaches the KL to a \emph{frozen} label-conditional prior --- an
orthonormal anchor frame for classification, fixed random-Fourier anchors for regression
--- reached through a side head that inference never reads. Because the reference cannot
move, the compression term becomes a squared distance, which supplies the strict
convexity that squared-IB installs by hand, and the fully compressed corner is a
class-separated configuration with cross-entropy $\ln(1 + (K{-}1)e^{-ca})$ rather than the
chance-level $\ln K$ of a marginal prior. Across 9 datasets, 4 backbones and 5{,}100
training runs, FGIB attains the best mean rank among seven objectives, and is the only
bottleneck in the study whose $\beta$ knob is both safe and useful: sweeping $\beta$ over
six decades costs FGIB at most $0.52$ accuracy points on every classification setting ---
where VIB, CEB and DVCCA instead fall to the trivial predictor on all of them --- and
FGIB is the only objective with a positive median gain over its own backbone. The
guarantee weakens on regression, where the continuous-anchor construction is a transfer
argument rather than a proof, and the side head that reaches the anchors remains a second
trainable party inside the KL: we keep an explicit ledger of what is proved and what is
open.
\end{abstract}
...
\emph{(P3) The variances race.} With both $q(z|x) = \Ncal(m(x), \sigma^2 I)$ and
$b(z|y) = \Ncal(g(y), \tau^2 I)$ trainable, the KL is stationary in $\sigma^2$ at
$\sigma^2 = \tau^2$ while the cross-entropy term pushes $\sigma^2$ down without
opposition once the means match; the equilibrium sits at the numerical clamp, where the
certificate has degenerated into a mean-matching force of weight $e^{10}$ between two
trainable maps (Proposition~\ref{prop:variance}).
...
(Remark~\ref{rem:A-channel}); the one-to-one sweep requires a bounded classifier scale
that the present implementation does not enforce (Remark~\ref{rem:c-scale}); and no
...
(Remark~\ref{rem:surrogate}). Section~\ref{sec:limits-main} and
...
cross-entropy term, however, is convex in $z$, so by Jensen
...
direction-of-force argument, not a convergence proof, and are directly falsifiable by
logging the trained posterior log-variance (Remark~\ref{rem:p3-status}); we flag this as
an unperformed measurement in Section~\ref{sec:notyet}.
...
$10{,}000/1{,}000/1{,}000$ split. Appendix~\ref{app:data} gives the full preprocessing.
...
same best epoch --- and, in the event that both tie, pessimistically toward the lower test
score, so that no tie can flatter a method. FGIB is thus selected over a $6\times9$ grid
where the other bottlenecks are
selected over a 6-point grid --- a search-budget advantage we return to below. Standard
...
Two qualifications are essential. First, \emph{the margins are small}. Excluding the two
...
\emph{FGIB's selection grid is $9\times$ larger}. We control for this in
Section~\ref{sec:knob} by also reporting FGIB at a single anchor scale fixed a priori,
which changes none of the conclusions there; but for Table~\ref{tab:main} specifically,
part of the rank advantage may be search budget.
...
moves from $98.15$ to $98.27$ across the entire six-decade sweep. The bottom row of
Table~\ref{tab:robust} shows the price: NIB's median gain at its \emph{best} $\beta$ is
$-0.27$ points. It does beat its own backbone on 4 of the 12 settings (by $+5.3$ on
AgeDB/MLP, $+2.4$ on California Housing, $+1.3$ on IMDb and $+0.1$ on MNIST/CNN), but it
loses on the other 8, so at the median the knob is not worth turning. The same row is
negative for VIB ($-0.15$), SVIB ($-0.14$), CEB ($-0.22$) and DVCCA ($-0.15$), and
positive only for FGIB ($+0.39$, or $+0.27$ at fixed $a=16$). \emph{Among the six
bottlenecks, FGIB is the only one whose compression knob is both non-destructive ---
uniformly so on classification, at the median overall --- and, at the median, worth
turning.}
...
Three experiments that this paper's own theory asks for are not in it, and we list them so
that the gap is on the record rather than in a reader's inference.
\emph{(i) The variance-race measurement.} Proposition~\ref{prop:variance}(ii)--(iii) is a
sharp, falsifiable hypothesis about CEB's training dynamics: log the mean posterior
log-variance and the fraction of units sitting at the clamp $e^{-10}$. If trained
variances settle at an interior value, that part of P3 is empirically void
(Remark~\ref{rem:p3-status}). \emph{(ii) The conditioning measurement.}
$\kappa(A^{\top}A)$ over training decides how large the side-path perturbation in
Result~\ref{res:equiv} actually is, and therefore whether the open channel of
Section~\ref{sec:limits-main}(a) bites in practice. \emph{(iii) Separating the
...
would test it. Beyond the theory's own asks, the robustness properties that motivate
...
configuration FGIB's compressed corner selects is the neural-collapse geometry
\citep{papyan2020prevalence}. What is new here is not the geometry but its role: it enters
...
that this single fact accounts for a dead knob, a blind certificate, a variance race and a
...
and, more distinctively, a compression knob that can be turned six decades without
destroying the model --- something no other objective in the comparison manages while also
beating its own backbone. The channel that remains open is the one the construction did
not remove: the side head is a second trainable party inside the KL, and constraining it,
or measuring how badly it degrades, is the natural next step.

\subsection*{AI use statement}

(This section is \textbf{required} and does not count toward the page limit.)

\textbf{[Authors: complete before submission.]} In this work, we used generative AI tools
for [tasks with required disclosure]. We have reviewed all AI-assisted work. We take
responsibility for the final content of this work, including text, claims or artifacts
produced with the aid of generative AI. See the ICLR 2027 AI Policy for Authors for more
details.
...
number in this paper is transcribed by hand. All proofs are in Appendix~\ref{app:theory}, with the assumptions of each
...
Appendix~\ref{app:data}. Model selection uses validation scores only; test scores are
...
\textbf{[To be written.]} This appendix should record, per dataset: source and version,
split sizes and the fixed seed, feature extraction (ResNet-50 layer-3 with $4\times4$
pooling for ImageNet-100; BERT-base layer-10 token features with PCA to 50 dimensions for
AG News; frozen GloVe-6B-100d embeddings for STS-B), target scaling (min-max to $[0,1]$
fitted on the training split only, with MAE reported in original units), graph
construction and normalization for Cora and ZINC, and the image pipeline for AgeDB.
