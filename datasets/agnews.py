...
def get_agnews_dataloaders(
    ...
    """加载 AG News BERT 特征并返回训练/验证/测试 DataLoader。

    官方 train/test 合并后 60/20/20 分层划分（seed 固定），特征一次性读入
    RAM 并转 fp32（约 25.5GB，本机 RAM 充裕）。layer 指定 BERT 层号
    （默认 10 = 倒数第三层，对应 agnews_bert_base_layer{layer}_features.npy）。
    random_labels=True 时把训练集标签按固定 seed 随机化（信息自由数据集
    I(X;Y)=0 的记忆实验），验证/测试集保持真实标签。
    """
    base = os.path.join(data_dir, "agnews")
    features = np.load(
        os.path.join(base, f"agnews_bert_base_layer{layer}_features.npy")
    )
