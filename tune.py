...
def build_tune_parser():
    ...
    replace_arg(
        parser, "model", ["--model"],
        nargs="+", choices=["mlp", "cnn", "gcn", "rnn", "vib", "ceb", "fgib", "fgibs", "svib", "nib", "dvcca",
                            "dceb", "tafgib", "centfgib", "etf"],
        default=["mlp"],
        help="模型列表，调参网格的一维；基础模型无 beta/anchor 维度，"
        "vib/ceb/svib/nib/dvcca/dceb 仅 beta 维度，fgib/fgibs/tafgib/centfgib 为 "
...
def main():
    ...
    if not ({"fgib", "fgibs", "tafgib", "centfgib"} & set(models)) and len(anchors) > 1:
        print(f"警告：模型列表中没有 fgib/fgibs/tafgib/centfgib，--anchor-scale 列表不会被使用")
    ...
    for model in models:
        ...
        elif model in ("fgib", "fgibs", "tafgib", "centfgib"):
