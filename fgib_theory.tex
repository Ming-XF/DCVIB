...
$\mathrm{CE}(c(z), y) = -\log \mathrm{softmax}_y(c(z))$ is convex in $z$ (log-sum-exp
minus a linear term), so Jensen gives
...
it is convex in $z$ and Jensen gives the stated inequality; its gradient on $\sigma^2$
is negative wherever the classifier is unsaturated. The KL's partial derivatives at the
...
$(m - g)/\tau^2$; once $\|m-g\| \to 0$ the race to the clamp is unopposed. For (iii): a deterministic continuous $z$
...
($S_\theta$ is a model output), and cannot be measured from the certificate during
training (only its existence can be inferred). The CE term presses the
...
the inference behavior (a conceptual limit: unless the means align perfectly, the
bound's \emph{value} $\|\mu - a v_y\|^2/(2\sigma_p^2)$ diverges as $\sigma_p^2 \to 0$;
the finite-covariance setting is what keeps the regularizer well-defined,
Remark~\ref{rem:limit}). CEB cannot take this limit: its noise is in the training path,
...
side-path noise is known and removable
...
\label{eq:fgib-ceb}
\end{equation}
That is, the side-path regularizer is pointwise identical to a CEB bottleneck with a
\emph{fixed} backward encoder, applied to
