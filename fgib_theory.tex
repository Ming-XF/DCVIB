...
% 核心结论（FGIB 论文问题陈述与解法）：
% 第一部分（条件瓶颈识别信息、不识别几何）：
%   P1 信息点退化（原"死旋钮"）：CEB ≡ IB(β=1+γ>1)，确定性场景下所有 γ>0 的唯一最优解
%      钉在角点即 MNI 点——目标函数只识别信息点，不识别该点上的表示几何；
%   prop:geomet 几何不可识别（第四轮新增核心命题）：MNI 解在任意可逆重编码下保持
%      信息点与最优 Bayes 交叉熵，但欧氏几何（类间距离/尺度/条件数）可在 GL(d) 内
%      任意改变——等价类中几何由优化器决定，不由目标函数决定；
%   P2 证书盲视 + 1:β 交换率：T_α 流形上 ReX 恒为 0，目标以 1 nat 标签信息 ↔
%      β nat 认证残差的比率交换（辅助论据：完整目标还含 CE 偏好保留标签）；
%   P3 方差竞赛（降级为参数化诊断）：KL 两侧皆可训练，分类项压 σ→0、钳位处退化为
%      权重 e^{10} 的均值匹配。仅限可训练先验族（CEB/DCEB），VIB 固定先验无此通道
%      （rem:vib-prior）；DCEB 先验方差方向为**膨胀**（cor:family，第四轮修正）；
%   P4 对象错位（降为 limitation）：界只界住含噪码、不界住部署表示的残差（+∞）。
...
% =====================================================================
\section*{Part I. Conditional bottlenecks identify information, not geometry}
% =====================================================================
...
$N \perp X \mid Y$, and every bijective recoding of these. Proposition~\ref{prop:geomet}
makes the point precise at the MNI solution: the objective is invariant under bijective
recodings while the Euclidean geometry --- inter-class distances, scale, conditioning ---
is arbitrary within the class. Which member of the class is reached is settled by the
...
\begin{proposition}[Geometric non-identifiability at the MNI solution]\label{prop:geomet}
Let $Z = \varphi(X)$ attain the MNI point of the CEB objective (Eq.~1), where the
objective is the \emph{ideal information-theoretic} one --- the proposition concerns
the objective, not any finite-capacity variational parametrization or optimization
trajectory. Then for every bijective measurable map $\psi$ with measurable inverse,
the recoded representation
$\psi(Z)$ attains the \emph{same} information point,
$I(X;\psi(Z)|Y) = I(X;Z|Y)$ and $I(Y;\psi(Z)) = I(Y;Z)$, and the optimal Bayes
cross-entropy is unchanged, $H(Y \mid \psi(Z)) = H(Y \mid Z)$: the objective identifies
an \emph{equivalence class} of representations, never a member of it. Within the
class, scale and conditioning can be made \emph{arbitrarily poor}: for
finite-dimensional codes (finite second moments) and invertible linear
$\psi = B \in \mathrm{GL}(d)$, the class-mean geometry
transforms as
$\|B(m_y - m_{y'})\|^2 = (m_y - m_{y'})^{\top} B^{\top}B\,(m_y - m_{y'})$;
taking $B = sI$ rescales every inter-class distance by $s$, and taking
$B = \mathrm{diag}(c, 1, \dots, 1)$ sends the condition number
$\kappa(B^{\top}B) \to \infty$ as $c \to \infty$. Inter-class distances, scale, and
conditioning of an MNI solution are therefore optimizer artifacts --- the objective
selects none of them. (A single $B$ cannot prescribe an arbitrary pairwise-distance
matrix; the claim is the direction-free statement that scale and conditioning are
unbounded within the class.)
\end{proposition}

\begin{proof}
Mutual information is invariant under bijections of either argument (data-processing
inequality applied in both directions). For the Bayes risk, any classifier $g$ on $Z$
induces $g \circ \psi^{-1}$ on $\psi(Z)$ with identical risk, and conversely, so
$\inf_f \mathbb{E}\,\ell(Y, f(\psi(Z))) = \inf_g \mathbb{E}\,\ell(Y, g(Z))$; in
log-loss form this reads $H(Y \mid \psi(Z)) = H(Y \mid Z)$. The geometry statement is
the displayed computation.
\hfill\qed
\end{proof}

\begin{remark}[Why the equivalence class is what FGIB designs]\label{rem:design}
Proposition~\ref{prop:geomet} separates the two levels the conditional bottleneck
confounds: the objective fixes the information point (what Proposition~\ref{prop:dead}
shows is a single corner in the deterministic scenario), and the optimizer alone picks
the member of the equivalence class --- geometry included. FGIB's fixed anchor prior is
a \emph{choice of representative}: with a frozen conditional prior $r(z|y)$ the
zero-penalty set $\{\operatorname{KL}(q\|r) = 0\}$ is exactly the anchor geometry, and
Proposition~\ref{prop:varfreeze} shows the deterministic main path freezes the variance
at the prior value, so the regularizer is identically the squared distance to a fixed,
well-conditioned, class-separated frame. FGIB therefore does not estimate information
better; it \emph{designs the zero-penalty set}, replacing an optimizer-settled geometry
with a designed one.
\end{remark}
...
\begin{remark}[The race needs a trainable prior; VIB is not in it]\label{rem:vib-prior}
The mechanism of Proposition~\ref{prop:variance} uses a trainable prior variance
$\tau^2$: the race is the joint descent of $\sigma^2$ (driven by the prediction term)
and $\tau^2$ (following the moment-matched value), which leaves the KL stationary
because only the \emph{ratio} $\sigma^2/\tau^2$ is penalized. VIB's prior is the fixed
unit Gaussian, so this channel is absent: with $\tau^2 = 1$ the KL is
$\tfrac{d}{2}\big[\sigma^2 + \|m\|^2/d - 1 - \ln\sigma^2\big]$ with restoring force
$\partial_{\sigma^2}\operatorname{KL} = \tfrac{d}{2}(1 - 1/\sigma^2) < 0$ for
$\sigma^2 < 1$ --- the KL pushes the posterior variance back \emph{up} toward $1$. VIB's
high-$\beta$ failure is a different mechanism: the fixed marginal prior pulls the
posterior mean to zero, $q(z|x) \to \mathcal{N}(0, I)$, so the stochastic code loses the
prediction information ($I(Y;Z) \to 0$) while the variance stays at the prior value.
An earlier version of this paper lumped ``VIB and CEB race to the clamp''; the correct
statement is that the clamp race is a property of trainable-prior bottlenecks (CEB,
DCEB), and VIB's collapse is a mean collapse. The long-horizon control of
Section~\ref{sec:ablation} measures both directions: CEB's log-variances drift down
while VIB's hold at zero.
\end{remark}
...
$s = e^{-10}$ during CEB training. \emph{We ran the test, on the full validation set of
every epoch} (aggregated over all batches; Section~\ref{sec:ablation} of the main
paper). On MNIST, across all six
$\beta$ settings and 5 seeds, the trained
...
epoch of every run). A 400-epoch control at $\beta\in\{1,10\}$ (no early stopping,
Section~\ref{sec:ablation}) resolves the status more precisely: the variance \emph{does}
drift, monotonically and in exactly the direction (i)--(ii) predict --- posterior
$\log\sigma^2$ ends at $-2.27$ nats ($\beta=1$) and $-1.20$ ($\beta=10$), prior
tracking --- but the clamp fraction remains $0.0$ and $-2.3$ nats is far from the
$e^{-10}$ terminal state of part~(iii). The race is therefore real but slow: invisible
at the protocol horizon, directionally confirmed at four times it, convergence beyond
the tested 400 epochs. That is precisely the conditional status this remark assigns to
Proposition~\ref{prop:variance}(iii): a direction-of-force statement whose terminal
state the longest available run has not reached, and the measurement itself --- not the
prediction --- as the finding. On the FGIB side the question does not arise as an
empirical one at all: Proposition~\ref{prop:varfreeze} proves the posterior variance is
pinned at $\tau^2$ by construction --- the flat zero line of the 400-epoch run --- and
Corollary~\ref{cor:family} shows the race can
re-enter only through a trainable prior log-variance --- DCEB's configuration --- whose
measured collapse at $\beta=10$ is consistent with that channel.
\end{remark}
...
The channels are addressed by the FGIB design as follows; two are closed by
construction, one holds conditionally, and the mismatch is only partially addressed. The precise
...
channel of Part~I that FGIB has not closed; the fixed-orthogonal configuration of
Proposition~\ref{prop:orth} below closes it by construction, and its conditioning is
measured for the trainable variant in the main paper
(Section~\ref{sec:ablation}); Section~\ref{sec:limits} collects it.
\end{remark}

\begin{proposition}[Fixed orthogonal side head: the regularizer anchors the deployed representation directly]\label{prop:orth}
Let the side head be a \emph{fixed orthogonal} map $A \in \mathbb{R}^{d\times d}$ with
$z = d$, and let $\{v_y\}$ be the anchor frame. Then:
\begin{itemize}
\item[(i)] \emph{Isometry.} $\|A h - a v_y\| = \|h - a\,A^{\top} v_y\|$, and
$\{A^{\top}v_y\}$ is orthonormal --- combined with
Proposition~\ref{prop:varfreeze}, the regularizer is exactly the fixed-anchor center
loss $\tfrac{1}{2\tau^2}\|h - a\,A^{\top}v_y\|^2$ on the \emph{deployed representation}
$h$ itself, with the rotated frame $\{A^{\top}v_y\}$.
\item[(ii)] \emph{Conditioning.} $A^{\top}A = I$ has $\kappa = 1$ at every step: the
pulled-back reference of Proposition~\ref{prop:fgib-ceb} is orthonormal at scale $a$ in
the induced metric, the perturbation of Proposition~\ref{prop:inverse} is exactly
$\tau\,\epsilon$ with unit covariance --- no blow-up --- and the counterexample of
Proposition~\ref{prop:conditioning}(iii) is excluded by construction.
\item[(iii)] \emph{Statistical equivalence.} For a Haar-distributed random orthogonal
$A$, the rotated frame $\{A^{\top}v_y\}$ is distributed identically to $\{v_y\}$ across
seeds, so the identity map $A = I$ is a representative canonical choice: the
``$A{=}I$'' configuration loses nothing over any other fixed orthogonal map.
\end{itemize}
\end{proposition}

\begin{proof}
(i) is the isometry of orthogonal maps: $\|Ah - av_y\|^2
= \|A(h - aA^{\top}v_y)\|^2 = \|h - aA^{\top}v_y\|^2$, and
$v_i^{\top}A A^{\top} v_j = v_i^{\top}v_j = \delta_{ij}$. (ii) $\kappa(I) = 1$; the
induced metric is $A^{\top}A = I$; the perturbation covariance is
$\tau^2(A^{\top}A)^{-1} = \tau^2 I$. (iii) The QR frame $\{v_y\}$ is constructed from a
standard normal matrix, whose distribution is invariant under the orthogonal group;
Haar-distributed $A$ maps it to another frame with the same uniform distribution on the
Stiefel manifold, so the seed distribution of the whole optimization problem --- encoder
initialization, anchor frame, classifier initialization --- is identical for $A=I$ and
for any random fixed orthogonal $A$.
\hfill\qed
\end{proof}
...
\begin{proposition}[A deterministic main path freezes the variance: the released KL is exactly a fixed-variance displacement term]\label{prop:varfreeze}
Consider FGIB with a deterministic main path (the classifier consumes $h$, never $z$),
a log-variance head $\ell_q(x) = W h(x) + b$ with zero initialization, a frozen prior
log-variance $\ell_p = \ln \tau^2 = 0$, and a first-order optimizer without weight
decay. Then $\ell_q \equiv \ell_p$ at \emph{every} training step and the regularizer is,
throughout training,
\begin{equation}
\operatorname{KL}(q\,\|\,r)
= \tfrac{1}{2\tau^2}\,\|\mu - \mu_p\|^2,
\label{eq:freeze}
\end{equation}
a fixed-variance displacement term --- the KL of Lemma~\ref{lem:lagrangian} evaluated
permanently at the variance optimum $\sigma^2 = \tau^2$, not merely in the limit.
The per-coordinate partial derivative
$\partial_{\ell_q}\operatorname{KL} = \tfrac12(e^{\ell_q-\ell_p}-1)$ vanishes at
$\ell_q = \ell_p$ independently of the mean gap; we state it only as the local reason
the zero-initialized point is an invariant manifold. We do \emph{not} claim that the
initialized point is a global attractor of the coupled system: under the shared
parametrization $\ell_q(x) = Wh + b$ the per-coordinate gaps are coupled through the
Gram matrix of the features and through the evolving encoder, and the argument below
controls no such term.
\end{proposition}

\begin{proof}
The per-coordinate diagonal KL
$\tfrac12\big[\ell_p - \ell_q + e^{\ell_q-\ell_p} + (\mu-\mu_p)^2 e^{-\ell_p} - 1\big]$
gives
$\partial_{\ell_q}\operatorname{KL} = \tfrac12(e^{\ell_q-\ell_p}-1)$,
which vanishes at $\ell_q = \ell_p$, independently of the mean gap. With $W=0$, $b=0$
and $\ell_p=0$ the head output is identically zero, so $\ell_q = \ell_p = 0$ at
initialization and the gradient of every head parameter is zero at the first step;
by induction the head output is identically zero at every subsequent step, since the
head parameters never move, and no weight decay or other variance-dependent term acts
on the head: $\ell_q \equiv 0$ for all time.
Substituting $\ell_q=\ell_p=0$ into the closed form yields
Eq.~\eqref{eq:freeze}.
\hfill\qed
\end{proof}

\begin{corollary}[The FGIB family is a center-loss family; a trainable prior variance re-opens the variance channel in the inflation direction]\label{cor:family}
The same argument pins the posterior variance in every FGIB-family variant, so with
$\tau^2 = 1$: \emph{FGIB} and \emph{CentFGIB} are exactly
$\mathrm{CE} + \tfrac{\beta}{2}\|\mu - a v_y\|^2$ (frozen anchors), and \emph{TAFGIB} is
exactly the same loss with the anchor means trainable --- the ablation table's
``coincidence'' is an identity, not an empirical observation. \emph{DCEB} is the single
exception that leaves the family, but not in the direction an earlier version of this
paper claimed. Its trainable prior log-variance has gradient
$\partial_{\ell_p}\operatorname{KL} = \tfrac12\big[1 - e^{\ell_q-\ell_p}
- (\mu-\mu_p)^2 e^{-\ell_p}\big]$, which at the zero-initialized point equals
$-\tfrac12(\mu-\mu_p)^2 < 0$: under first-order \emph{descent},
$\ell_p^{t+1} = \ell_p^t - \eta\,\partial_{\ell_p}\operatorname{KL}$,
a negative derivative drives $\ell_p$ \emph{up}, so $\tau^2 = e^{\ell_p}$ grows and the
mean-matching coefficient $e^{-\ell_p}$ in the anchor force
$\nabla_h \operatorname{KL} = e^{-\ell_p}(\mu - \mu_p)$ \emph{decays}. Along the
ratio-pinned direction $\ell_q = \ell_p$ the KL itself decays as
$\tfrac12(\mu-\mu_p)^2 e^{-\ell_p} \to 0$ (the KL is homogeneous in the ratio
$\sigma^2/\tau^2$ and penalizes only the mean gap at unit ratio): the trainable prior
opens a variance-\emph{inflation} escape that \emph{dilutes} the regularizer, the
opposite of the amplification claimed before. The measured trajectory confirms the
sign ($\ell_p$ rises at $\beta=1$, never falls), but DCEB's $\beta=10$ collapse
(Section~\ref{sec:ablation} of the main paper) occurs with the variance near
initialization, so this channel does \emph{not} explain the collapse; its precise
mechanism remains an open, measured diagnostic (Section~\ref{sec:ablation},
Remark~\ref{rem:p3-status}). TAFGIB, whose prior variance is frozen, stays in the
family and does not collapse.
\end{corollary}
...
the initial regularization pressure; larger $a$ delays the approach to $s^*$. In the
released implementation the variance does not merely \emph{start} there: by
Proposition~\ref{prop:varfreeze} it is pinned there for the whole run, and
$\operatorname{KL} = \tfrac{1}{2\tau^2}\|\mu - \mu_p\|^2$ exactly. The
...
reference side (Prop.~\ref{prop:conditioning}(i)--(ii)); (8) the deterministic main path
freezes the variance: in the released implementation the regularizer is exactly the
fixed-variance displacement term $\tfrac{1}{2\tau^2}\|\mu-\mu_p\|^2$ at every step, and
the FGIB family is one center-loss family --- the ablation table's number-coincidence is
an identity, not an empirical observation (Prop.~\ref{prop:varfreeze},
Cor.~\ref{cor:family}); (9) the fixed orthogonal side head anchors the deployed
representation directly: $\|Ah - av_y\| = \|h - aA^{\top}v_y\|$ with $\kappa(A^{\top}A)
= 1$ and unit-covariance pull-back noise, so the conditioning counterexample is excluded
by construction and $A = I$ is a representative canonical choice
(Prop.~\ref{prop:orth}).

\textbf{Measured.} (i) The trainable side head $A$ degrades $\kappa(A^{\top}A)$ from
$10^{3.9}$ at initialization to $10^{5.8}$--$10^{6.8}$ (transient peaks to $10^{8.3}$)
on MNIST/MLP, without touching accuracy (main paper, Section~\ref{sec:ablation},
Fig.~\ref{fig:diag}) --- the channel of Remark~\ref{rem:A-channel} is real. The
fixed-orthogonal variant of Prop.~\ref{prop:orth} closes it by construction, at a
measured price: across five settings FGIB-H ($A{=}I$) trails the trainable head by
$\approx 0.5$ points per setting at the inherited anchor scale $a=16$ (mean rank
$4.00$ of $5$ vs.\ FGIB's $1.20$), recovering to mean rank $2.25$ when the anchor
scale is re-tuned for $h$-space ($a=4$), so the trade-off is
accuracy-versus-certification, small in both directions, and both configurations ship
(main paper, Table~\ref{tab:hanchor}). (ii) CEB's variance race is invisible at the protocol
horizon but drifts into view by 400 epochs, in the predicted direction and far from the
clamp (Remark~\ref{rem:p3-status}).

\textbf{Open or conditional.} (1) For the \emph{optional} trainable-$A$ variant, the
bound's object may be far from the deployed representation exactly where the residual
lives (Remark~\ref{rem:A-channel}, Prop.~\ref{prop:conditioning}(iii)); the
fixed-orthogonal configuration removes the issue by construction, and the measured
question left open is only whether trainable $A$ ever \emph{helps} --- on MNIST it does
not.
...
re-derived; the enforced-scale variants are tested on MNIST
(Section~\ref{sec:ablation} of the main paper). (3) Neither CEB's nor FGIB's certificate
upper-bounds the deployed
...
not certified semantic compression. (5) CEB's two-stage variance dynamics
(Prop.~\ref{prop:variance}(ii)--(iii)) remain a hypothesis for horizons beyond the
tested 400 epochs and for protocols other than ours; the FGIB-family counterpart is
proved (Prop.~\ref{prop:varfreeze}), with DCEB the single variant that re-opens the
race through its trainable prior variance (Cor.~\ref{cor:family}). (6) The regression
RFF anchor analysis inherits the same
fixed-scale caveats and is presently a transfer argument, not a proof; the
subject-disjoint AgeDB control (main paper, Table~\ref{tab:agedb-disjoint}) is
