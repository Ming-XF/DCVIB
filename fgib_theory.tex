...
Let $Z = \varphi(X)$ attain the MNI point of the CEB objective (Eq.~1). Then for every
bijective measurable map $\psi$ with measurable inverse, the recoded representation
...
an \emph{equivalence class} of representations, never a member of it. The Euclidean
geometry within the class is correspondingly unconstrained: for finite-dimensional
codes and invertible linear $\psi = B \in \mathrm{GL}(d)$, the class-mean geometry
transforms as
$\|B(m_y - m_{y'})\|^2 = (m_y - m_{y'})^{\top} B^{\top}B\,(m_y - m_{y'})$.
Taking $B = sI$ rescales every inter-class distance by $s$; taking
