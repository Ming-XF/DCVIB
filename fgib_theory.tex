...
% =====================================================================
\section*{Part I. Three failure channels of the conditional bottleneck and an object mismatch}
% =====================================================================
...
More generally, when the variance enters the objective only through the KL, the
per-coordinate gap $u = \ell_q - \ell_p$ obeys
$\partial_u \operatorname{KL} = \tfrac12(e^{u} - 1)$, which carries the sign of $u$:
$u = 0$ is the unique attractor of the continuous descent flow
$\dot u \propto -(e^{u}-1)$, and of discrete first-order updates whenever the standard
step-size condition holds; zero initialization selects this fixed point at the first
step and no force ever leaves it.
\end{proposition}
...
which vanishes iff $\ell_q = \ell_p$, independently of the mean gap. With $W=0$, $b=0$
and $\ell_p=0$ the head output is identically zero, so $\ell_q = \ell_p = 0$ at
initialization, the gradient of every head parameter is zero, and no weight decay or
other variance-dependent term acts on the head: $\ell_q \equiv 0$ for all time.
Substituting $\ell_q=\ell_p=0$ into the closed form yields
Eq.~\eqref{eq:freeze}. For the general attractor statement, the gap evolves under
gradient flow on $\ell_q$ as $\dot u = -\tfrac{\beta}{2}\|h\|^2(e^{u}-1)$, whose only
fixed point is $u=0$ and whose linearization is $-\tfrac{\beta}{2}\|h\|^2$, so $u=0$
is globally asymptotically stable (a one-dimensional monotone flow); a discrete
first-order step multiplies the gap by $1 - \tfrac{\eta\beta}{2}\|h\|^2 \tfrac{e^{u}-1}{u}$,
which contracts $|u|$ whenever $\eta \beta \|h\|^2 < 4/\sup_{u'} \tfrac{e^{u'}-1}{u'}$
--- the standard condition.
\hfill\qed
\end{proof}

\begin{corollary}[The FGIB family is a center-loss family; the variance race re-enters only through a trainable prior]\label{cor:family}
...
exception that leaves the family: its trainable prior log-variance has gradient
...
$-\tfrac12(\mu-\mu_p)^2 < 0$ --- the prior variance is pushed \emph{down} while the
means are unmatched, amplifying the mean-matching force by up to $e^{10}$ at the clamp.
This is Channel~P3 re-entering through the prior side, and it is consistent with the
measured behavior: DCEB is the only variant that collapses ($\beta=10$,
Section~\ref{sec:ablation} of the main paper), while TAFGIB, whose prior variance is
frozen, stays in the family and does not.
\end{corollary}
