\begin{table}[t]
\caption{The anchor scale controls the fully compressed corner. Test score of FGIB at the strongest compression pressure in the grid ($\beta=10$), as the anchor scale $a$ varies; $\Delta$ is the gap from the deterministic backbone at the largest anchor ($a{=}16$). Units as in Table~\ref{tab:main}. At small $a$ the anchors approach the single origin of a VIB prior and the model collapses to the trivial predictor (Remark~\ref{rem:a0}); growing $a$ recovers the backbone, and the 100-class task needs the largest $a$, as the $\ln(1+(K-1)e^{-ca})$ corner value predicts.}
\label{tab:anchor}
\begin{center}\small
