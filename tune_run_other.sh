...
# 2. 压缩-鲁棒性评估补训（文末）：MNIST β 网格保存到 output/adv_mnist。
...
# checkpoint 保存到 output/adv_mnist/{combo}/{combo}_run{i}.pt（+ train.log）。
...
python tune.py --task mnist --backbone mlp --model mlp ceb \
  ...
  --results-dir output/adv_mnist --save-root output/adv_mnist \
  --parallel 8 --runs 5 --epochs 100
